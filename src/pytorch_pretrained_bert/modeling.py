...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states,  theta, beta, topic_embedding, topic_mode, attention_mask, history_states=None, mask_qkv=None, seg_ids=None):
        ...
        query_layer = self.transpose_for_scores(mixed_query_layer, mask_qkv) #[batch,12,192,64] 其中12是attention_head_num, 64是attention_hidden_state
        ...
        attention_scores = torch.matmul(
            query_layer / math.sqrt(self.attention_head_size), key_layer.transpose(-1, -2)) #[batch,12,192,192] =[batch,12,192,64] * [batch,12,64,192]
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def beam_search(self, input_ids, theta, beta, topic_embedding, topic_mode, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        # print("attention_mask.size()", attention_mask, attention_mask.size())
        # print("task_idx.size()", task_idx, task_idx.size())
        # print("mask_qkv.size()", mask_qkv, mask_qkv.size())
        # exit()
        # input_ids #[64,121]
        # token_type_ids #[64,153] 153=121+32 输入加输出的长度，其中元素都是4和5 4表示输入句子，5表示输出句子。
        # position_ids #[64,153] 就是顺序的position, 如输入长度不足最大长度121，则编号不增加，从输出第一位开始
        # attention_mask #[64,153,153] 在input的时候把output都mask掉，在ouput的时候能看到自己和前面的，相当于是对角矩阵
        # task_idx #[64] 元素都是3
        # mask_qkv None
        input_shape = list(input_ids.size()) #[64,121] 121是这个批次最大的句子长度
        batch_size = input_shape[0] #64
        input_length = input_shape[1] #121
        output_shape = list(token_type_ids.size()) #[64,153] 153=121+32 输入加输出的长度，其中元素都是4和5 4表示输入句子，5表示输出句子。
        output_length = output_shape[1] #[153]

        output_ids = []
        prev_embedding = None
        prev_encoded_layers = None
        curr_ids = input_ids
        mask_ids = input_ids.new(batch_size, 1).fill_(self.mask_word_id) #mask_word_id =103 所以mask_ids = [64,1] 每个元素都是103 mask的token是 [MASK]
        next_pos = input_length
        if self.pos_shift: #False
            sos_ids = input_ids.new(batch_size, 1).fill_(self.sos_id)
        ...
        while next_pos < output_length:
            ...
            if self.pos_shift:# False
                ...
                start_pos = next_pos - curr_length #0
                x_input_ids = torch.cat((curr_ids, mask_ids), dim=1) #[64,122] #加入mask id 就是要预测那一个 #[320,2]

            curr_token_type_ids = token_type_ids[:, start_pos:next_pos + 1]#[64,122+1]

            curr_attention_mask = attention_mask[:,
                                                 start_pos:next_pos + 1, :next_pos + 1] #[64,122+1,122+1] #[320,2,123]
            # print("x_input_ids", x_input_ids.size(), x_input_ids)
            ...
            # print("new_embedding", new_embedding.size())
            # print("new_encoded_layers", new_encoded_layers[0].size())
            
            last_hidden = new_encoded_layers[-1][:, -1:, :] #[64,1,768]
            prediction_scores, _ = self.cls(
                last_hidden, None, task_idx=task_idx) #[64,1,28996] #词表中每个词都有
            log_scores = torch.nn.functional.log_softmax(
                prediction_scores, dim=-1)
            if forbid_word_mask is not None: #None
                log_scores += (forbid_word_mask * -10000.0)
            if self.min_len and (next_pos-input_length+1 <= self.min_len):# min_len = None
                log_scores[:, :, self.eos_id].fill_(-10000.0)
            if self.not_predict_set: #None
                for token_id in self.not_predict_set:
                    log_scores[:, :, token_id].fill_(-10000.0)
            kk_scores, kk_ids = torch.topk(log_scores, k=K) #[64,1,5] 每个批次选前5个最大的scores和词的id
            # print("last_hidden", last_hidden.size())
            # print("prediction_scores", prediction_scores, prediction_scores.size())
            # print("log_scores", log_scores, log_scores.size())
            # print("forbid_word_mask", forbid_word_mask)
            # print("min_len", self.min_len)
            # print("self.not_predict_set", self.not_predict_set)
            # print("kk_scores", kk_scores, kk_scores.size())
            # print("kk_ids", kk_ids, kk_ids.size())
            # exit()



            if len(total_scores) == 0:
                k_ids = torch.reshape(kk_ids, [batch_size, K]) #[batch, 5]
                back_ptrs = torch.zeros(batch_size, K, dtype=torch.long) #[batch, 5]
                k_scores = torch.reshape(kk_scores, [batch_size, K]) #[batch, 5]
            else:
            ...
            beam_masks.append(torch.eq(k_ids, self.eos_id).float()) #[batch,5] 看k_ids哪些元素到达了eos_id
            # print("torch.eq(k_ids, self.eos_id).float()", torch.eq(k_ids, self.eos_id).float(), torch.eq(k_ids, self.eos_id).float().size())
            # exit()
            total_scores.append(k_scores)
            def first_expand(x): #按照beam_size 放大，[64,121,768]->[320,121,768]
                input_shape = list(x.size()) #[64,121,768] 把mask那个维度去掉了
                expanded_shape = input_shape[:1] + [1] + input_shape[1:] #[64,1,121,768]
                x = torch.reshape(x, expanded_shape)
                repeat_count = [1, K] + [1] * (len(input_shape) - 1)#[1,5,1,1]
                x = x.repeat(*repeat_count) 
                x = torch.reshape(x, [input_shape[0] * K] + input_shape[1:]) #[320,121,768]
                return x
            ...
            def select_beam_items(x, ids):
                ...
                return y
            is_first = (prev_embedding is None) #True
            ...
            if self.pos_shift:# False
                ...
                if prev_encoded_layers is None:
                    ...
                    prev_encoded_layers = [select_beam_items(
                        x, back_ptrs) for x in prev_encoded_layers]
            curr_ids = torch.reshape(k_ids, [batch_size * K, 1]) #[batch*5] #按顺序铺开了
