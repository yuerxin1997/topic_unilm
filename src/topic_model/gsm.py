...
class GSM(nn.Module):
    def __init__(self, encode_dims=[2000,500,20],decode_dims=[20,500,2000],dropout=0.8):
        ...
        self.hidden_encoder = nn.Linear(encode_dims[0],encode_dims[1]) #[2000,500]
        self.fc_mu = nn.Linear(encode_dims[1],encode_dims[2]) #[500,20]
        self.fc_logvar = nn.Linear(encode_dims[1],encode_dims[2]) #[500,20]

        self.hidden_decoder_1 = nn.Linear(decode_dims[0],decode_dims[1]) #[20,500]
        self.hidden_decoder_2 = nn.Linear(decode_dims[1],decode_dims[2]) #[500,2000]
        self.dropout = nn.Dropout(p=dropout)
        self.fc1 = nn.Linear(encode_dims[-1],encode_dims[-1])

        # self.v  = nn.Linear( decode_dims[-2], decode_dims[-1]) #[3007, 512] 是反着来的
        # self.t = nn.Linear(decode_dims[-2], decode_dims[0]) #[20, 512]
        # nn.init.xavier_uniform_(self.v.weight)
        # nn.init.xavier_uniform_(self.t.weight)
        # self.t = torch.empty(decode_dims[0], 500).cuda() #[ topics, 100]
        # self.v = torch.empty(decode_dims[-1], 500).cuda() #[ |V|, 100] 是反着来的
        # nn.init.xavier_normal_(self.t)
        # nn.init.xavier_normal_(self.v)
        self.v  = nn.Embedding( decode_dims[-1], decode_dims[-2]) #[3007, 512] 是反着来的
        self.t = nn.Embedding(decode_dims[0], decode_dims[-2]) #[20, 512]
        nn.init.xavier_uniform_(self.v.weight)
        nn.init.xavier_uniform_(self.t.weight)
        self.theta = None
        self.beta = None
    ...
    def encode(self, x):
        hid = F.relu(self.dropout(self.hidden_encoder(x))) #[batch,500] = [batch,2000] * [2000,500]
    ...
    #     return p_x
    def decode(self, theta): #theta = [batch, topics]
        self.beta = torch.mm(self.t.weight, self.v.weight.transpose(0,1)) #[topic, |v|] = [20, 500] * [500 * 2000]
    ...
    def forward(self, x, use_gsm=True):
        ...
        return x_reconst, mu, log_var, self.theta, self.beta
